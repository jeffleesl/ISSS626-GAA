...
This exercise is a continuation of **Exercise 10.1: Processing and Visualising Flow Data** and the following data will be used:
...
```{r}
mpsz_sp <- as(mpsz, "Spatial") #convert sf to sp
mpsz_sp
```
...
1.  **Simplified Calculations:** Instead of calculating distances between all possible points on each polygon’s boundary, using centroids reduces it to a single point per polygon.

2.  **Representation of the "Center" of Each Area:** The centroid serves as a good approximation of the "central" or "average" location within the subzone, making it useful for many spatial analyses, including clustering, accessibility analysis, and distance-based modeling.

3.  **Comparability:** Centroid-to-centroid distances provide a consistent basis for comparison between zones, especially when calculating Euclidean distances, which are straightforward and computationally efficient.
:::
...
```{r}
distPair <- melt(dist) %>% #dplyr better version reshape2 is the grandfather of dplyr
  rename(dist = value)
head(distPair, 10)
```
...
```{r}
# should not use this
# no of school counts (pri,sec and jc)
flow_data1 <- flow_data1 %>%
  left_join(pop,
            by = c(DESTIN_SZ = "SZ")) %>%
  rename(DESTIN_AGE7_12 = AGE7_12, # destination is the receiving zone
...
![](images/clipboard-2098303418.png){width="475"}

The code chunk used to calibrate to model is shown below:

```{r}
uncSIM <- glm(formula = TRIPS ~ # take note is glm and not lm
                log(ORIGIN_AGE25_64) +  #origin should be positive
                log(DESTIN_AGE25_64) + #destin should be positive
                log(dist), # distance should be negative
...
```{r}
r2_mcfadden(uncSIM) # available in performance package, can be used for logistic and Poisson regression models
```
...
![](images/clipboard-2418230122.png){width="490"}
...
![](images/clipboard-787820714.png){width="498"}
...
![](images/clipboard-1522313502.png){width="493"}

```{r}
# create Doubly Constrained
