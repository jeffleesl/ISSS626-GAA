...
tourism_sf_avg_2023 <- st_as_sf(tourism_sf_avg_2023)
...
```{r}
tourism_sf_2019 <- tourism_sf_2019[!st_is_empty(tourism_sf_2019$geometry), , drop = FALSE]
```

```{r}
class(tourism_sf_2019) # check if the class is an sf object
```

```{r}
glimpse(tourism_sf_2019) #confirm that the geometry column exists and is an sfc (simple feature geometry)
```
