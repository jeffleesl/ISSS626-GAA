...
```{r}
pacman::p_load(sf, spatstat, tmap, spNetwork, tidyverse)
set.seed(1234)
```
...
Using a Kernel Density Estimation (KDE) to identify high-risk areas.
...
```{r}
# Quadrat counting for hotspots
quadrat_counts <- quadratcount(ppp_accidents, nx = 10, ny = 10)

# Plot quadrat counts
plot(quadrat_counts, main = "Hotspots based on Quadrat Counting")
...
# 7. Geovisualisation and Geocommunication

```{r}

# Visualize accidents with the road network and administrative boundaries
tm_shape(tsab_bmr) + 
  tm_polygons() + 
  tm_shape(hotosm_road_types) + 
  tm_lines(col = "grey") + 
...
# Map accidents by behavioral factors (e.g., speeding)
tm_shape(tsab_bmr) +
  tm_polygons() +
...
# Map accidents by environmental factors (e.g., rainy conditions)
tm_shape(tsab_bmr) +
  tm_polygons() +
...
# Adaptive KDE for hotspots
adaptive_kde <- density(ppp_accidents, sigma = bw.scott(ppp_accidents))  # Use Scott's bandwidth

plot(adaptive_kde, main = "Adaptive KDE for Accident Hotspots")

```

```{r}

# 3D plot for accident density
plot_ly(x = ~coords_accidents[,1], y = ~coords_accidents[,2], z = ~accident_density$z, 
        type = "mesh3d")
